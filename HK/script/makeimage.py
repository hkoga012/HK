...
#作成前に既存ファイル削除
subprocess.run("del /Q *HFS-R1_*.*", stdout=subprocess.PIPE, shell=True)

#--------------------------------------------------------------------
#ファームウェエア更新用モトローラ形式ファームウェアファイル作成
#対象ツール：HFS-R1_CommTool.exe(メンテツール)
#ファイル名：HFS-R1_Ver99.99.9999.mot (9の部分は取得した数値 0サプレス)
#開始アドレス：0xFFFE0000 (従来仕様に合わせるための開始アドレス)
#出力ROMサイズ：0x20000　(128Kbyte)
#--------------------------------------------------------------------
mot_filename = "HFS-R1_Ver" + major.zfill(2) + "." + minor.zfill(2) + "_Rev" + revision.zfill(4) + ".mot"
res = subprocess.run(BIN2MOT + " R1.bin.signed " + mot_filename + " 0xFFFE0000 0x20000")
if 0!=res.returncode:
...
#ファイル名：Factory_HFS-R1_Ver99.99.9999.mot (9の部分は取得した数値 0サプレス)
#開始アドレス：0x00000000 (ROM開始アドレス)
#出力ROMサイズ：0x21800　 (134Kbyte = 120Kbyte(R1.bin.signed) + 14Kbyte(Bootloader.bin))
#--------------------------------------------------------------------
#Bootloader.bin の後ろに R1.bin.signedを追加し、ROMイメージ作成
img_filename = "HFS-R1_Image.bin"
res = subprocess.run("copy /y /b Bootloader.bin + R1.bin.signed " + img_filename, stdout=subprocess.PIPE, shell=True)
if 0!=res.returncode:
     print("#### ERROR #### Failed to create ROM Image file(bin)")
else:
     #結合したバイナリをモトローラ形式へ変換
     imgmot_filename = "Factory_HFS-R1_Ver" + major.zfill(2) + "." + minor.zfill(2) + "_Rev" + revision.zfill(4) + ".mot"
