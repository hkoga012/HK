	...
	:BaseTask(ST_DATA_MAX, EVT_DATA_MAX, TIMER_DATA_MAX, DATA_EVENT_QUE_SIZE, _pStateString, _pEventString)
{
    ...
    st->AddEventHandler(EVT_DATA_NFC_STARTED, WaitReaderStarted_NfcReaderStarted);  //!< NFCリーダ起動完了
...
@author
-------------------------------------------------*/
void DataManager::WaitReaderStarted_NfcReaderStarted(BaseTask* pOwnTask, uint32_t EventId, const uint8_t* pData, uint32_t DataLen)
{
    (void)EventId; (void)pData; (void)DataLen;  // 未使用引数のワーニング対策
	DataManager* pMyTask = (DataManager*)pOwnTask;
...
/*!------------------------------------------------　
@brief 最短起動時間を超えていたら、コマンド受信タスクを開始　//要確認
@note　　
@return なし
@author
-------------------------------------------------*/
...
//    DebugCommand::SendPrintSci("..._IfDoneShortestTimeToStartCommandReceiver()\r\n");
	...
	{
		#ifdef NFC_ERROR_DEBUG_RW_INIT_ERROR
    		_IsNfcStarted = 0;
		#endif
	    if (_IsNfcStarted)
		{
		...
		else			// NFCリーダが初期化に失敗しているので再起動
		{
            //ICリーダー初期化エラー+1 1万回を超える場合はカウントしない。 変更点
            if(G_pDataManager->GetCount_EVENT_Reader_InitERROR()<=10000){
				G_pDataManager->AddEventCount(EVENT_ERROR_RW_INIT);
...
ENUM_SEL_FUNC_FELICA DataManager::GetSelFuncFelica()
{
    ...
    {
        case 0xf: ret = SEL_FUNC_FELICA_FTSIC_24; break;       // 1111
        ...
        case 0x5: ret = SEL_FUNC_FELICA_FTSIC24_IDM; break;   // 0101
        //case 0x0: ret = SEL_FUNC_FELICA_INITIAL_COM; break; // 0000
        case 0x0: ret = SEL_FUNC_FELICA_FTSIC_2; break; // 0000
        case 0x4: ret = SEL_FUNC_FELICA_FTSIC2_IDM; break;   // 0101
    ...
    switch (GetSelFuncFelica()) {
		...
		*/
        case SEL_FUNC_FELICA_NO_READ:
            ret = (((_DipSw1Info & 0x3d) == 0x31) ? SEL_FUNC_MIFARE_READ : SEL_FUNC_MIFARE_NO_READ);
            break;
        default:
            ret = ((_DipSw1Info & 0x01) ? SEL_FUNC_MIFARE_READ : SEL_FUNC_MIFARE_NO_READ);
            break;
    }
...
@brief 機能選択 入退室判定
@note
@return 1:退室側(0x4f:I) 0:入室側(0x49:O)
@author
-------------------------------------------------*/
uint8_t DataManager::IOFunc()
{
	return (_DipSw1Info & 0x80) ? 0x4f : 0x49;
}
...
/*!------------------------------------------------
@brief システム情報をROMに書き込みリブート（書くかどうかは、DataManagerの状態に依存する）　//要確認
@note
@return なし
@author
-------------------------------------------------*/
...
/*!------------------------------------------------
@brief 前回終了時のLED表示状態、ブザー音をOutputControllerに通知する (変更点　LEDは消灯)
@note　
@return なし
@author
-------------------------------------------------*/
...
//	DebugCommand::SendPrintSci("NoticeInitLedInfoToOutputController()\n");
	...
	G_pOutputController->LEDoff();
	static const uint8_t Buzzercommand = 'A';//起動コマンド
	G_pOutputController->SendEvent(EVT_OUTPUT_BUZZER,&Buzzercommand,LEN_BUZZER_IN_INST_CMD);
}
...
/*
	...
	if (GetVeeRecId() == 1)
	{
		...
		// 前回終了時データデバッグ用データ
		pObj->info.ArrayLedStatusForInit[0] = 'L'; // 赤
		pObj->info.ArrayLedStatusForInit[1] = 'K'; // 緑
		//pObj->info.ArrayLedStatusForInit[2] = 'V'; // 青
		pObj->info.ArrayLedStatusForInit[3] = 'M'; // 橙
	}
...
void BaseInDataFlash::Update()
{
    ...
    fsp_err_t err_t = RM_VEE_FLASH_Open(g_vee0.p_ctrl, g_vee0.p_cfg);
    //ClsCommon::PrintInfoNum(DebugCommand::SendPrintSci, "\r\nRM_VEE_FLASH_Open() = ", err_t);  //変更点　元コメント
    if(FSP_SUCCESS == err_t)
    {
        __debugCounterDataFlashCheckd ++;
        ...
        err_t = RM_VEE_FLASH_RecordPtrGet(g_vee0.p_ctrl, GetVeeRecId(), &pBufOnFlash, &len);
        //ClsCommon::PrintInfoNum(DebugCommand::SendPrintSci, "\r\nRM_VEE_FLASH_RecordPtrGet() =", err_t); //変更点　元コメント
        if (FSP_SUCCESS == err_t)
		{
            ClsCommon::PrintInfoNum(DebugCommand::SendPrintSci, "len = ", len); //変更点　元コメント

            if (len == GetRecSize())
			{
	            //ClsCommon::PrintInfoHex(DebugCommand::SendPrintSci, "pBufOnFlash = ", (uint32_t)pBufOnFlash); //変更点　元コメント
			...
			//ClsCommon::PrintInfoNum(DebugCommand::SendPrintSci, "isChanged = ", isChanged); //変更点　元コメント
		}
