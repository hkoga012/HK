	...
	:BaseTask(ST_OUTPUT_MAX, EVT_OUTPUT_MAX, TIMER_OUTPUT_MAX, OUTPUT_EVENT_QUE_SIZE, _pStateString, _pEventString)
{
	...
	for(uint8_t i=0;i<LED_IDX_MAX-1;i++)
	{
...
{
	OutputController* pMyTask = (OutputController*)pOwnTask;
	uint8_t Command = (uint8_t)'N';	//起動時コマンド'N'
	...
	PatIndex = pMyTask->GetLedPatternIndex(Command);
		TYPE_LED_CTRL* p = &pMyTask->_ledControl[3];
		p->pCurrent = (DATA*)&pMyTask->_ledPattern[PatIndex][0];
		p->Command = Command;
		p->PatternIndex = 0;
		p->EndCommand = 0;
...
void OutputController::Running_Led(BaseTask* pOwnTask, uint32_t EventId, const uint8_t* pData, uint32_t DataLen)
{
    ...
    for(uint8_t i=0;i<LED_IDX_MAX-1;i++)
	{
		...
		if(PatIndex >= 0)
		{
		    ...
		    if((Command >= 'K' && Command <= 'M') || (Command == '0'))
		    {
...
void OutputController:: DetectedCard(bool IsDetect)
{
    ...
    if (_isDetectedCard != pLEDCtrl->PrevIsDetectedCard)
    {
        ...
        LEDOutput(LED_IDX_FRAME, pLEDCtrl->LastData.Ctrl);
    }
}
*/
		...
		else
		{
			if (pLEDCtrl->WorkData.Count > 0)
			{
				...
				if (pLEDCtrl->WorkData.Count == pLEDCtrl->pCurrent[pLEDCtrl->PatternIndex].Count)
				{
					...
					err = LEDOutput(LedIndex, pLEDCtrl->WorkData.Ctrl);
...
@author
-------------------------------------------------*/
TYPE_ERROR OutputController::LEDOutput(uint8_t LedIndex, ENUM_CTRL Data)
{
    TYPE_ERROR err = E_OK;
	...
	*/
    ...
    {
        case IO_RED:	//! 赤100%点灯
			err = LEDRGB(LedIndex, 0, 100);
            break;
        case IO_GRN:	//! 緑100%点灯
    		err = LEDRGB(LedIndex, 100, 0);
            break;
		/*
        case IO_BLE:	//! 青100%点灯
    		err = LEDRGB(LedIndex, 0, 0, 100);
            break;
		*/
        case IO_ORG:	//! 橙(赤100%、緑25%点灯)　要確認　色味調整
            err = LEDRGB(LedIndex, 0, 75);
            break;
		/*
        case IO_WHE:	//! 白(赤100%、緑100%、青100%)
    		err = LEDRGB(LedIndex, 100, 100, 100);
            break;
		*/
        case IO_OFF:	//! 消灯(赤0%、緑0%)
    		err = LEDRGB(LedIndex, 100, 100);
            break;
    	case IO_KEP:
    	case IO_END:
    		err = E_OK;
...
@author
-------------------------------------------------*/
//TYPE_ERROR OutputController::LEDRGB(uint8_t LedIndex, uint8_t DutyR, uint8_t DutyG, uint8_t DutyB)
TYPE_ERROR OutputController::LEDRGB(uint8_t LedIndex, uint8_t DutyR, uint8_t DutyG)
{
	TYPE_ERROR err = E_OK;
	...
	//PWM *pPWM_R = NULL;
	PWM *pPWM_G = NULL;
	//PWM *pPWM_B = NULL;           //R1環境で要確認
	bsp_io_port_pin_t PortR = LED_1_R;
	//bsp_io_port_pin_t PortB = LED_Left_B;
	//if(DutyR > 100 || DutyG > 100 || DutyB > 100)
	if(DutyR > 100 || DutyG > 100)
	{
		return E_PAR;
	}
	///////////////////////
    ...
    case LED_IDX_0:
    	//LED_Left
    	PortR = LED_1_R;
		pPWM_G = G_pDrvPWM_LED_1_G;
		//PortB = LED_Left_B;
    	break;
    case LED_IDX_1:
    	//LED_Center
    	PortR = LED_2_R;
		pPWM_G = G_pDrvPWM_LED_2_G;
		//PortB = LED_Center_B;
    	break;
    case LED_IDX_2:
    	//LED_Right
    	PortR = LED_3_R;
		pPWM_G = G_pDrvPWM_LED_3_G;
		//PortB = LED_Right_B;
    	break;
    case LED_IDX_P:
    	PortR = LED_P_R;
		pPWM_G = G_pDrvPWM_LED_P_G;
	...
	*/
	...
	//緑出力
	if(pPWM_G==G_pDrvPWM_LED_1_G || pPWM_G==G_pDrvPWM_LED_P_G){
		err = pPWM_G->SetDutyA(DutyG);
		}else {
			err = pPWM_G->SetDutyB(DutyG);
		}
...
int8_t OutputController::GetLedPatternIndex(char Id)
{
    ...
    {
    	index = (int8_t)(Id - 'A') + 0x0A; //10-25を返す
    }
	/*
    else if(Id >= 'a' && Id <= 'b')
    {
		...
		if (_buzzerControl.WorkData.Count > 0)
		{
			...
			if (_buzzerControl.WorkData.Count == _buzzerControl.pCurrent[_buzzerControl.PatternIndex].Count)
			{
				...
				if (IO_OFF == _buzzerControl.WorkData.Ctrl)
				{
					//出力OFF
#ifndef WINDOWS_DEBUG
				    err = G_pDrvPWM_Buzzer->SetDutyA(0);
#else
				...
				else
				{
				    ...
				    {
				        // 毎回ボリュームのAD値を読んで、その割合をブザーのデューティ比としてセットし音量を変える　変更点（論理反転）
				        uint16_t duty = G_pDrvADC_Vol->ReadRate(MIN_VOL_AD_RANGE, MAX_VOL_AD_RANGE);
                        uint8_t duty2 = (uint8_t)((duty*duty)/142);  // ボリュームの変化に対し、累乗した値をデューティ比とし、音量低部分の変化を合わせれるようにする
				        err = G_pDrvPWM_Buzzer->SetDutyA(duty2);
...
/*!------------------------------------------------
@brief 電源投入時にLEDを消灯する。 変更点
@note
@return なし
@author
-------------------------------------------------*/
void OutputController::LEDoff()
{
    uint8_t Command = (uint8_t)'0'; //消灯コマンド'0'
    int8_t PatIndex = G_pOutputController->GetLedPatternIndex(Command);
	for(uint8_t i=0;i<3;i++)
	{
		TYPE_LED_CTRL* p = &G_pOutputController->_ledControl[i];
		p->pCurrent = (DATA*)&G_pOutputController->_ledPattern[PatIndex][0];
		p->Command = Command;
		p->PatternIndex = 0;
		p->WorkData = p->pCurrent[p->PatternIndex];
		p->EndCommand = Command;
	}
}
