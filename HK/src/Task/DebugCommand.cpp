...
void DebugCommand::SendPrintSci(const char* buf)
{
	...
	//G_pDrvUART_RS485->Write((uint8_t*)buf, (uint32_t)strlen(buf));
#endif
}


/*!------------------------------------------------
@brief シリアルポート出力 printfフォーマット対応機能追加 変更点
...
void DebugCommand::SendPrintSciEx(const char* format, ...)
{
	...
	//G_pDrvUART_RS485->Write((uint8_t*)tmpBuffer, (uint32_t)cnt);
}
#endif
...
uint32_t DebugCommand::GetReceiveData(char* str, uint32_t DataLen)
{
	...
	//return G_pDrvUART_RS485->Read((uint8_t*)str, DataLen);
#endif
}
...
void DebugCommand::Stop_Start(BaseTask* pOwnTask, uint32_t EventId, const uint8_t* pData, uint32_t DataLen)
{
	...
	myTask->_pTaskCmdParam[3] = {"DataManager",      G_pDataManager};
	myTask->_pTaskCmdParam[4] = {"FwUpdater",        G_pFwUpdater};
	myTask->_pTaskCmdParam[5] = {"CommandReceiver",  G_pCommandReceiver};
	myTask->_pTaskCmdParam[6] = {"EventSender",      G_pEventSender};
	myTask->_pTaskCmdParam[7] = {NULL,               NULL};    //終端(削除禁止)
}
