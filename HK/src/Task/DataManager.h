...
typedef enum {
    ...
    SEL_FUNC_FELICA_FTSIC_2,
    SEL_FUNC_FELICA_FTSIC_24,          //!< FTSic(2,4 ブロック)、FTSicカード以外は無応答
    SEL_FUNC_FELICA_SURPASS,        //!< サーパス向け(2,4 ブロック) 、サーパスカード以外は無応答
    SEL_FUNC_IDM,                   //!< IDm
    SEL_FUNC_FELICA_FTSIC2_IDM,      //!< FTSic(2 ブロック) + IDm 、FTSicカード以外はIDmを応答
    SEL_FUNC_FELICA_SURPASS_IDM,    //!< サーパス向け(2,4 ブロック) + IDm  、サーパスカード以外はIDmを応答
    SEL_FUNC_FELICA_FTSIC24_IDM,      //!< FTSic(2,4 ブロック) + IDm 、FTSicカード以外はIDmを応答
...
typedef enum : uint16_t {
    ...
    , ERRORFLAG_PORT_RELAY_OFFSET = 2   //! リレー出力異常
    , ERRORFLAG_PORT_RW_OFFSET = 3      //! ICリーダ リセットポート出力異常
    , ERRORFLAG_PORT_BLE_OFFSET = 4     //! 【未使用】BLE異常 変更点
    , ERRORFLAG_HOST_PRI_OFFSET = 8     //! RS485 上位プライマリ回線通信エラーイベント
    , ERRORFLAG_HOST_SEC_OFFSET = 9     //! 【未使用】RS485 上位セカンダリ回線通信エラーイベント
    , ERRORFLAG_RW_OFFSET = 10          //! ICリーダ 通信回線通信エラーイベント
    //, ERRORFLAG_QR_OFFSET = 11        //! 【未使用】QRリーダ 回線通信エラーイベント
    , ERRORFLAG_BLE_OFFSET = 11         //! 【未使用】BLE
} ERRORFLAG_OFFSET;

typedef enum : uint8_t {
    ERRORFLAG_PORT_485_DE_A = 0x80      //! RS485 Primary DEポート出力異常
    , ERRORFLAG_PORT_485_RE_A = 0x40    //! RS485 Primary REポート出力異常
    , ERRORFLAG_PORT_485_DE_B = 0x20    //【未使用】RS485 Secondary DEポート出力異常
    , ERRORFLAG_PORT_485_RE_B = 0x10    //【未使用】RS485 Secondary REポート出力異常
    , ERRORFLAG_PORT_485_FET = 0x08     //! RS485 FETポート出力異常　変更点
    ...
    , ERRORFLAG_QR_INIT = 0x80          //! 【未使用】QRリーダ 通信初期化エラー
    , ERRORFLAG_QR_OVER = 0x40          //! 【未使用】QRリーダ 通信受信オーバーランエラー
    , ERRORFLAG_QR_PERR = 0x20          //! 【未使用】QRリーダ 通信受信パリティエラー(未使用：パリティ無し)
    , ERRORFLAG_QR_FERR = 0x10          //! 【未使用】QRリーダ 通信受信フレーミングエラー
} ERRORFLAG;
...
typedef enum {  // 空きは予備、１つの項目は2byteで構成される
    ...
    , EVENT_SENDQR                 	    //  7:【未使用】QRリーダ 読み取りデータ 正常送信回数
    , EVENT_SENDLIDOPEN         	    //  8:蓋開データ送信回数
    , EVENT_SENDLIDCLOSE        	    //  9:蓋開データ送信回数
    ...
    , EVENT_ERROR_QR_INIT = 24          // 24:【未使用】QRリーダー初期化異常検出(タイムアウト)
    , EVENT_ERROR_QR_OVER               // 25:【未使用】QRリーダー通信オーバーラン検出回数
    , EVENT_ERROR_QR_PER                // 26:【未使用】QRリーダー通信パリティエラー検出回数
    , EVENT_ERROR_QR_FER                // 27:【未使用】QRリーダー通信フレーミングエラー検出回数
    ...
    , EVENT_ERROR_HOST_NO_COM = 40      // 40:上位から指示コマンドなし
...
constexpr uint8_t LEN_INI_MIFARE_KEY = 6;       //! 【未使用】イニシャル設定 読取位置キーデータ
constexpr uint8_t LEN_INI_FERICA_SYSTEM = 2;    //! 【未使用】イニシャル設定 システムコード
constexpr uint8_t LEN_INI_FERICA_SERVICE = 2;   //! 【未使用】イニシャル設定 サービスコード
...
constexpr uint8_t INITIAL_MIFARE_KEYA = '0';      // 【未使用】イニシャル設定 MIFARE KEYA
constexpr uint8_t INITIAL_MIFARE_KEYB = '1';      // 【未使用】イニシャル設定 MIFARE KEYA
constexpr uint8_t INITIAL_FERICA_VALID = 'V';     // 【未使用】イニシャル設定 FERICA 有効
constexpr uint8_t INITIAL_FERICA_INVALID = 0x20;  // 【未使用】イニシャル設定 FERICA 無効(設定用)
...
public:
    ...
    static void WaitReaderStarted_NfcReaderStarted(BaseTask* pOwnTask, uint32_t EventId, const uint8_t* pData, uint32_t DataLen); // リーダ起動完了
    ...
    uint8_t IOFunc();               //入退室判定
    uint8_t GetReaderAddress();     // リーダーアドレスを取得
    //uint8_t GetReaderHwStatus();    // リーダーハードウエア状態を取得　→削除
    ...
    //変更点　ICリーダーの初期化回数カウント
    uint16_t GetCount_EVENT_Reader_InitERROR()
    {
        uint8_t* pVal_H = (uint8_t*)(&_DataStoredInDataFlash.info.MemoryBuf[EEPROM_MAP_COUNTER_OFFSET + (EVENT_ERROR_RW_INIT * 2)]);
        uint8_t* pVal_L = pVal_H + 1;
        uint16_t val = (uint16_t)(*pVal_H << 8) + *pVal_L;  //値に変換する
        return val;
    }
