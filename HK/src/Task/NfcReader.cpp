...
*/
    ...
    //DIPSW1読取り対象カード指定
    uint8_t Target = pMyTask->GetTargetCode( G_pDataManager->GetSelFuncMifare(), G_pDataManager->GetSelFuncFelica() );
    //uint8_t Target = pMyTask->GetTargetCode(G_pDataManager->GetSelFuncFelica());
...
void NfcReader::Running_NoticeTimer(BaseTask* pOwnTask, uint32_t EventId, const uint8_t* pData, uint32_t DataLen)
{
    ...
    else
    {
        ...
        G_pDrvPWM_Buzzer->SetDutyA(0); // 定期的に止める
        G_pDrvPWM_LED_Frame_B->SetDutyA(0);   // Frame LED ON
#endif
    }
...
#ifdef DEBUG_NFC_POWER_DIPSW //　DIPSWでアンテナを調整時、ビープ音を鳴らす
    G_pDrvPWM_Buzzer->SetDutyA(50); // 検出時鳴らす
    G_pDrvPWM_LED_Frame_B->SetDutyA(100);   // Frame LED OFF
#endif
...
//uint8_t NfcReader::GetSCode(uint8_t CardType, ENUM_SEL_FUNC_FELICA FuncFeliCa)
{
    ...
    if(IS_MIFARE(CardType))
    {
        ...
        */
       if(FuncMifare == SEL_FUNC_MIFARE_READ){
            SCode = ((IS_MIFARE_1K(CardType) || IS_MIFARE_4K(CardType)) ? NFC_SCODE_MIFARE : NFC_SCODE_NONE);
       }
    }
    else if(IS_FELICA(CardType))
    {
        ...
        case SEL_FUNC_FELICA_FTSIC_24: SCode = (IS_FELICA_FTS(CardType) ? NFC_SCODE_FTSIC : NFC_SCODE_NONE); break;

        // FTSic(2 ブロック)、FTSicカード以外は無応答
        case SEL_FUNC_FELICA_FTSIC_2: SCode = (IS_FELICA_FTS(CardType) ? NFC_SCODE_FTSIC : NFC_SCODE_NONE); break;

        // サーパス向け(2 ブロック) 、サーパスカード以外は無応答
        ...
        case SEL_FUNC_FELICA_FTSIC24_IDM: SCode = (IS_FELICA_FTS(CardType) ? NFC_SCODE_FTSIC : NFC_SCODE_IDM); break;

        // FTSic(2 ブロック) + IDm 、FTSicカード以外はIDmを応答
        case SEL_FUNC_FELICA_FTSIC2_IDM: SCode = (IS_FELICA_FTS(CardType) ? NFC_SCODE_FTSIC : NFC_SCODE_IDM); break;

        // サーパス向け(2 ブロック) + IDm  、サーパスカード以外はIDmを応答
        ...
        */
        ...
        case SEL_FUNC_FELICA_FTSIC_24:
        case SEL_FUNC_FELICA_FTSIC_2:
        SCode = (IS_FELICA_FTS(CardType) ? NFC_SCODE_Felica : NFC_SCODE_NONE); break;

        case SEL_FUNC_FELICA_SURPASS:
        SCode = (IS_FELICA_SP(CardType) ? NFC_SCODE_Felica : NFC_SCODE_NONE); break;

        case SEL_FUNC_IDM:
        SCode = NFC_SCODE_IDM; break;

        case SEL_FUNC_FELICA_FTSIC24_IDM:
        case SEL_FUNC_FELICA_FTSIC2_IDM:
        SCode = (IS_FELICA_FTS(CardType) ? NFC_SCODE_Felica : NFC_SCODE_IDM); break;

        case SEL_FUNC_FELICA_SURPASS_IDM:
        SCode = (IS_FELICA_SP(CardType) ? NFC_SCODE_Felica : NFC_SCODE_IDM); break;
...
//uint8_t NfcReader::GetTargetCode(ENUM_SEL_FUNC_FELICA FuncFeliCa)
{
...
void NfcReader::DebugPrintSendData(uint8_t *pData, uint8_t DataLen)
{
    ...
    switch(pData[0])
    {
        ...
        case NFC_SCODE_SURPASS:
            ...
            break;
        */
       case NFC_SCODE_Felica:
            pSendType = "Felica";  //!< SURPASS
            break;
