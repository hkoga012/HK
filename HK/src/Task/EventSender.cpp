...
void EventSender::WaitOrder_ReqResetData(BaseTask* pOwnTask, uint32_t EventId, const uint8_t* pData, uint32_t DataLen)
{
	...
	switch (EventId)
	{
	    case EVT_EVTSND_REQ_RESET_DATA_POWER:
			...
			int t;
			if(G_pDataManager->IOFunc() == 0x4f){
				t = 25;
				} else {
					t=0;
				}

	        pMyTask->StartTimer(EVT_EVTSND_REQ_RESET_DATA_POWER_TIMER, (G_pDataManager->GetReaderAddress()+1)*100+t);
            return;
        /*
...
void EventSender::Sending_ReqResetData(BaseTask* pOwnTask, uint32_t EventId, const uint8_t* pData, uint32_t DataLen)
{
	...
	else									//衝突していない場合
	{
		...
		{
		*/
	    if (EventId == EVT_EVTSND_REQ_READ_DATA)
	    {
			//ステータス要求コマンド受信時
			pMyTask->SendReqResetData(0x30);
		}
...
void EventSender::SendReqReadData(const uint8_t* pData)
{
    ...
    //I/O(入退室判定)
    _SendedDataBuf[0] = G_pDataManager->IOFunc();

	//読み取ったデータの識別用コード
	//変更点　読み取りデータのフォーマット

	if(*pData == 0x32){ //Mifareの場合
		...
		_SendedDataBufLen = DT_LEN_REQ_READ_24;
		this->SendData(DT_LEN_REQ_READ_24);
	} else { //Felicaの場合
			ENUM_SEL_FUNC_FELICA iCode = (G_pDataManager->GetSelFuncFelica());
			switch (iCode)
			{
				case SEL_FUNC_FELICA_FTSIC2_IDM:
				case SEL_FUNC_FELICA_SURPASS_IDM:
				_SendedDataBuf[1] = *pData; break;

				case SEL_FUNC_FELICA_FTSIC24_IDM:
				if(*pData == 0x30)
				{
					_SendedDataBuf[1] = 0x12; break;
				} else{
					_SendedDataBuf[1] = *pData; break;
				}

				default: _SendedDataBuf[1] = 0x12; break;
			}
		...
		switch (iCode)
		{
			...
			if(_SendedDataBuf[1] == 0x12)
			{
				...
				//DLE(固定)
				_SendedDataBuf[18] = 0x10;
				//未使用(固定)
				_SendedDataBuf[19] = 0x30;
				//ボタン情報(固定)
				_SendedDataBuf[20] = 0x30;
				//送信済みデータ長変数に長さをセット
				_SendedDataBufLen = DT_LEN_REQ_READ_2;
				//上位システムにデータ送信
				this->SendData(DT_LEN_REQ_READ_2);
			} break;
...
void EventSender::SendReqResetData(uint8_t Factor)
{
	...
	//送信済みデータバッファを初期化する
	ResetSendedDataBuf();
	//製品識別コード(固定：0x51)　adrs:[0] →入退室判定（I/O）
	_SendedDataBuf[0] = G_pDataManager->IOFunc();
	//データ送信要因　adrs:[1]
	...
	ClsCommon::I2HexAsciiByte(G_pDataManager->GetInfoDipSw2(), &dummy, (char*)&_SendedDataBuf[6]);
...
void EventSender::SendedDataCheck()
{
    ...
    {
        case(DT_LEN_REQ_READ_2)://読み取りデータ
		case(DT_LEN_REQ_READ_24):
            ...
            else*/
            {
				...
				G_pDataManager->AddEventCount(EVENT_SENDLIDCLOSE); //変更点
            }
            break;
