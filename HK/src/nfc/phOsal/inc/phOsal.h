...
/**
 * \brief This function allocates the secure stack to the calling thread.
 *
 *
 * @param[in]   stackSizeInNum    Stack size to be allocated for the calling thread in secure context.
 *
 * @return Status of the API
 * @retval #PH_OSAL_SUCCESS              Operation successful.
 * @retval #PH_OSAL_UNSUPPORTED_COMMAND  Command not supported.
 */
extern phStatus_t phOsal_ThreadSecureStack(
            uint32_t stackSizeInNum);
