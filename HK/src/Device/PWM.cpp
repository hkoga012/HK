...
@author
-------------------------------------------------*/
TYPE_ERROR PWM::SetDutyA(uint8_t Duty)
{
    fsp_err_t err = FSP_SUCCESS;
...
/*!------------------------------------------------
@brief デューティ比設定（GTIOCBにも出力する為追加）             変更点
@param Duty 0-100で指定(単位％)
@return エラーコード
@author
-------------------------------------------------*/
TYPE_ERROR PWM::SetDutyB(uint8_t Duty)
{
    fsp_err_t err = FSP_SUCCESS;
	timer_info_t info = {(timer_direction_t)0, 0, 0};
	uint32_t DutyCycle = 0;
	...
	//出力先はGTIOCBのみ
	err = R_GPT_DutyCycleSet(_pTimerCtrl, DutyCycle, GPT_IO_PIN_GTIOCB);
    if(FSP_SUCCESS != err)
    {
    	return E_SYS;
    }
